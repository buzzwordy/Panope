...
export interface PanopeApi {
  ...
  countResource(resourceKey: string): Promise<{ total: number; byNs: Record<string, number>; partial?: boolean } | null>
  ...
  /** OpenAPI v3 schemas for an apiVersion (field completion). null if unavailable. */
  openApiSchemas(apiVersion: string): Promise<Record<string, unknown> | null>
  /** group/versions this cluster serves, for the API removal scan. */
  servedApiVersions(): Promise<string[]>
  ...
  // AI assistant (desktop only). Reads run immediately with the user's
  // identity; mutations pause on a confirmation card in the panel.
  aiSend(text: string, ctx?: AiUiContext): Promise<void>
  aiStop(): Promise<void>
  /** `alwaysTool` records an external tool as pre-approved for future calls. */
  aiConfirm(id: string, approve: boolean, alwaysTool?: string): Promise<void>
  aiReset(): Promise<void>
  /** Replay of the persisted transcript (events), for panel remount and app restart. */
  aiHistory(): Promise<AiEvent[]>
  /** Model ids offered by an endpoint, for the settings dropdown. */
  aiListModels(cfg: { provider: string; baseUrl?: string; apiKey?: string }): Promise<string[]>
  /** Connection state of each configured external MCP server. */
  aiMcpStatus(): Promise<McpServerState[]>
  aiGetConfig(): Promise<AiConfig | null>
  aiSetConfig(cfg: {
    provider: string
    baseUrl?: string
    model: string
    ...
    /** send a pruned list to revoke a remembered external tool; omit to keep */
  ...
  /** Ask GitHub for the newest release. Never rejects; check `error`. */
  checkForUpdate(): Promise<UpdateCheck>
  /** newest patch release for a Kubernetes minor, or null. */
  latestK8sPatch(minor: string): Promise<string | null>

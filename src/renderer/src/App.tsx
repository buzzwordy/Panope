...
import type { ClusterInfo, K8sObject, KubeContextInfo, UpdateCheck } from '@shared/types'
...
import { KubeconfigsModal } from './components/modals/KubeconfigsModal'
import { PreferencesModal } from './components/modals/PreferencesModal'
import { AboutModal } from './components/modals/AboutModal'
import { ShortcutsModal } from './components/modals/ShortcutsModal'
import { COMPUTE, type StatusValue, type StatusVariant } from './lib/accessors'
import { useResourceData } from './hooks/useResourceData'
import { useCounts, displayCount } from './hooks/useCounts'
import { useCatalog } from './hooks/useCatalog'
import { usePortForwards } from './state/portForwards'
import { useToast } from './state/toast'
import { loadPrefs, savePrefs, themeBase, resolveDataTheme, type SavedView, type ThemeId } from './lib/prefs'
import { UpdateBanner } from './components/UpdateBanner'
import { initAssistantStore } from './state/assistant'
import { versionSupport, supportLabel, minorOf, patchesBehind } from './lib/k8sVersions'
...
/** Sun/moon toggle: flip light<->dark while keeping the palette family. */
function flipTheme(t: ThemeId): ThemeId {
  switch (t) {
    case 'system': return themeBase('system') === 'dark' ? 'light' : 'dark'
    case 'dark': return 'light'
    case 'light': return 'dark'
    case 'github-dark': return 'github-light'
    case 'github-light': return 'github-dark'
  }
}
...
export function App(): React.ReactElement {
  ...
  const [statusFilter, setStatusFilter] = useState<Set<StatusVariant>>(new Set())
  const [updateCheck, setUpdateCheck] = useState(PREFS.updateCheck)
  const [refreshSeconds, setRefreshSeconds] = useState(PREFS.refreshSeconds)
  const [k8sLatest, setK8sLatest] = useState<string | null>(null)
  const [update, setUpdate] = useState<UpdateCheck | null>(null)
  const [aiOpen, setAiOpen] = useState(false)
  const [splitDetail, setSplitDetail] = useState<DetailEntry | null>(null)
  ...
  useEffect(() => {
    ...
    startMetricsHistory()
    // assistant events accumulate here even while the panel is closed
    initAssistantStore()
  ...
  // Patch currency for the running minor. Same opt-out as the release check,
  // and silent on failure: an air-gapped cluster should not see an error it
  // cannot act on.
  useEffect(() => {
    const minor = minorOf(clusterInfo?.version)
    if (!updateCheck || !isDesktop || !minor) return
    let disposed = false
    void api.latestK8sPatch(minor).then((v) => !disposed && setK8sLatest(v))
    return () => {
      disposed = true
    }
  }, [clusterInfo?.version, updateCheck])

  // One release check per launch, after the cluster has had a moment to load.
  // Failures stay silent - an offline or proxied machine shouldn't see an error
  // it can do nothing about. A version the user skipped is never re-announced.
  useEffect(() => {
    if (!updateCheck || !isDesktop) return
    const t = setTimeout(() => {
      void api.checkForUpdate().then((u) => {
        if (u.newer && u.latest !== loadPrefs().updateSkipped) setUpdate(u)
      })
    }, 4000)
    ...
    const unsub = api.onMenuAction((action) => {
      ...
      switch (action) {
        ...
        case 'assistant':
          ...
          break
        case 'check-updates':
          // Asked for explicitly, so say something either way.
          void api.checkForUpdate().then((u) => {
            if (u.newer) setUpdate(u)
            else if (u.error) toast.error(`Could not check for updates: ${u.error}`)
            else toast.success(`Panope ${u.current} is the latest release`)
          })
          break
      }
    })
  ...
  const { items, loading, error, metrics, refresh } = useResourceData(def, contextVersion, refreshSeconds)
  ...
  function onPalettePick(key: string): void {
    ...
    if (key.startsWith('cmd:')) {
      // Same handlers the native menu uses; the palette is just another way in.
      switch (key.slice(4)) {
        case 'create': openCreate(); break
        case 'export-csv': exportCsv(); break
        case 'refresh': refresh(); break
        case 'toggle-theme': setTheme(flipTheme); break
        case 'toggle-density': setDensity((d) => (d === 'compact' ? 'comfortable' : 'compact')); break
        case 'portforwards': setPfOpen(true); break
        case 'preferences': setMenu('preferences'); break
        case 'shortcuts': setMenu('shortcuts'); break
        case 'kubeconfigs': setMenu('kubeconfigs'); break
        case 'readonly': void toggleReadOnly(); break
      }
      return
    }
    if (key.startsWith('ctx:')) {
      void onContextChange(key.slice(4))
      return
    }
    if (key.startsWith('ns:')) {
      setNamespace(key.slice(3))
      return
    }
  ...
  // Follow the OS while the theme is 'system' (re-render the app so the
  // resolved base reaches CodeMirror and the sun/moon icon too).
  ...
  const paletteItems = useMemo<PaletteItem[]>(() => {
    const out: PaletteItem[] = [
      { key: 'overview', label: 'Cluster Overview', group: '', icon: 'cube' },
      ...
      ...(isDesktop ? [{ key: '__assistant', label: 'Assistant', group: '', icon: 'sparkles' }] : [])
    ]
    for (const d of catalog.byKey.values()) out.push({ key: d.key, label: d.label, group: d.category, icon: d.icon })
    // Commands and switches after the kinds: an empty query stays a navigation
    // list, and typed queries surface these through the palette's ranking.
    out.push(
      { key: 'cmd:create', label: 'Create resource...', group: 'Commands', icon: 'plus' },
      { key: 'cmd:refresh', label: 'Refresh current view', group: 'Commands', icon: 'refresh' },
      { key: 'cmd:export-csv', label: 'Export list to CSV', group: 'Commands', icon: 'save' },
      { key: 'cmd:toggle-theme', label: 'Toggle theme', group: 'Commands', icon: 'sun' },
      { key: 'cmd:toggle-density', label: 'Toggle row density', group: 'Commands', icon: 'sliders' },
      { key: 'cmd:portforwards', label: 'Port forwards...', group: 'Commands', icon: 'forward' },
      { key: 'cmd:kubeconfigs', label: 'Kubeconfigs...', group: 'Commands', icon: 'config' },
      { key: 'cmd:readonly', label: readOnly ? 'Disable read-only mode' : 'Enable read-only mode', group: 'Commands', icon: readOnly ? 'unlock' : 'lock' },
      { key: 'cmd:preferences', label: 'Preferences...', group: 'Commands', icon: 'sliders' },
      { key: 'cmd:shortcuts', label: 'Keyboard shortcuts', group: 'Commands', icon: 'list' }
    )
    for (const c of contexts) {
      if (c.name !== clusterInfo?.context) {
        out.push({ key: `ctx:${c.name}`, label: `Switch context: ${c.name}`, group: 'Contexts', icon: 'layers' })
      }
    }
    for (const n of ['All', ...nsList]) {
      if (n !== namespace) out.push({ key: `ns:${n}`, label: `Namespace: ${n}`, group: 'Namespaces', icon: 'namespace' })
    }
    // live objects last - typing a pod name still finds it directly
    out.push(...paletteObjItems)
    return out
  }, [catalog, paletteObjItems, contexts, clusterInfo?.context, nsList, namespace, readOnly])
  useEffect(() => {
  ...
  const statusCol = useMemo(() => def?.columns.find((c) => c.kind === 'status'), [def])
  const statusOf = useCallback(
    (o: K8sObject): StatusVariant | null => {
      if (!statusCol) return null
      const v = COMPUTE[statusCol.id]?.(o)
      return v && typeof v === 'object' && 'variant' in v ? (v as StatusValue).variant : null
    },
    [statusCol]
  )
  ...
  // Chip counts come from the pre-status list, so a chip never hides itself.
  const statusCounts = useMemo(() => {
    const counts = new Map<StatusVariant, number>()
    if (!statusCol) return counts
    for (const o of baseFiltered) {
      const v = statusOf(o)
      if (v) counts.set(v, (counts.get(v) ?? 0) + 1)
    }
    return counts
  }, [baseFiltered, statusCol, statusOf])

  const filtered = useMemo(() => {
    if (!statusFilter.size || !statusCol) return baseFiltered
    return baseFiltered.filter((o) => {
      const v = statusOf(o)
      return v !== null && statusFilter.has(v)
    })
  }, [baseFiltered, statusFilter, statusCol, statusOf])
  ...
  function applySavedView(v: SavedView): void {
    ...
    setStatusFilter(new Set((v.statusFilter ?? []) as StatusVariant[]))
    if (def) {
      setHiddenCols((prev) => {
        const next = { ...prev }
  ...
  function saveCurrentView(): void {
    ...
    const view: SavedView = {
      ...
      statusFilter: statusFilter.size ? [...statusFilter] : undefined
    }
    setSavedViews((prev) => {
  ...
  const favItems = useMemo<FavItem[]>(() => {
    const special: Record<string, FavItem> = {
      overview: { key: 'overview', label: 'Cluster Overview', icon: 'chart' },
  ...
  // Drag the divider between the sidebar and the content; clamp and persist.
  const startSidebarResize = (e: React.MouseEvent): void => {
    ...
    const up = (): void => {
      ...
      if (!Number.isNaN(w)) savePrefs({ sidebarWidth: w })
    }
  ...
  return (
    <div
      className={`app-shell${aiOpen && isDesktop ? ' has-assistant' : ''}${splitDetail ? ' has-split' : ''}`}
    ...
    >
      ...
      />
      <div className="sidebar-resizer" onMouseDown={startSidebarResize} title="Drag to resize" />
      ...
      <div className="main-column">
        <TopBar
          ...
          onToggleTheme={() => setTheme(flipTheme)}
        ...
        {update && (
          <UpdateBanner
            update={update}
            onSkip={() => {
              savePrefs({ updateSkipped: update.latest })
              setUpdate(null)
            }}
        ...
        {activeKey === 'fleet' && !detail ? (
          ...
          <>
            ...
            <RightSizingView
              ...
              onOpen={(pod, tab) => {
                const d = catalog.byKey.get('pods')
                if (d) setDetailStack([{ def: d, obj: pod, tab }])
              }}
            />
          </>
          ...
          <>
            ...
            <div className="toolbar">
              ...
              {statusCol && statusCounts.size > 1 && (
                <div className="status-chips">
                  {(['failed', 'pending', 'unknown', 'running', 'succeeded'] as const).map((v) => {
                    const n = statusCounts.get(v)
                    if (!n) return null
                    const on = statusFilter.has(v)
                    return (
                      <button
                        key={v}
                        className={`status-chip is-${v}${on ? ' is-on' : ''}`}
                        title={`${on ? 'Stop showing only' : 'Show only'} ${v} rows`}
                        onClick={() =>
        ...
        <footer className="footer">
          ...
          {clusterInfo?.version && (
            <span>
              ...
              {(() => {
                const v = versionSupport(clusterInfo.version)
                const behind = patchesBehind(clusterInfo.version, k8sLatest ?? undefined)
                // The minor's support state outranks patch currency: being on
                // the newest patch of a dead minor is still a dead minor.
                if (v && (v.state === 'eol' || v.state === 'ending')) {
                  const note =
                    behind === null
                      ? ''
                      : behind === 0
                        ? ` You are on the newest ${v.minor} patch (${k8sLatest}).`
                        : ` You are also ${behind} patches behind (${k8sLatest}).`
                  return (
                    <span className={`ver-badge is-${v.state}`} title={supportLabel(v) + note}>
                      {v.state === 'eol' ? 'unmaintained' : 'ends soon'}
                    </span>
                  )
                }
                if (behind && behind > 0) {
                  return (
                    <span
                      className="ver-badge is-ending"
                      title={`${behind} patch release${behind > 1 ? 's' : ''} behind. Newest is ${k8sLatest}.`}
                    >
                      {behind} behind
                    </span>
                  )
      ...
      {splitDetail && (
        <aside className="split-pane">
          <div className="split-pane__head">
            <Icon name={splitDetail.def.icon} size={14} />
            <span className="split-pane__title">
              {splitDetail.def.kind} · {splitDetail.obj.metadata?.name}
            </span>
            <span className="spacer" />
            <button className="icon-btn" title="Close split" onClick={() => setSplitDetail(null)}>
              <Icon name="close" size={14} />
            </button>
          </div>
          <div className="split-pane__body">
            <DetailView
              key={splitDetail.obj.metadata?.uid ?? splitDetail.obj.metadata?.name}
              def={splitDetail.def}
              obj={splitDetail.obj}
              now={now}
              theme={themeBase(theme)}
              contextVersion={contextVersion}
              initialTab={splitDetail.tab as never}
              readOnly={readOnly}
              onBack={() => setSplitDetail(null)}
              onDrill={(d, o) => setSplitDetail({ def: d, obj: o })}
              onChanged={refresh}
            />
          </div>
        </aside>
      ...
      {aiOpen && isDesktop && (
        <AssistantPanel
          uiCtx={{
            ...
            namespace,
            view: activeKey,
            kind: detail?.def.kind,
            name: detail?.obj.metadata?.name,
            objNamespace: detail?.obj.metadata?.namespace
          }}
      ...
      {menu === 'kubeconfigs' && <KubeconfigsModal onClose={() => setMenu(null)} />}
      ...
      {menu === 'preferences' && (
        <PreferencesModal
          ...
          onSetRefreshSeconds={(n) => {
            setRefreshSeconds(n)
            savePrefs({ refreshSeconds: n })
          }}
          onSetTheme={setTheme}
          onToggleReadOnly={toggleReadOnly}
          onToggleUpdateCheck={() =>
            setUpdateCheck((v) => {
              savePrefs({ updateCheck: !v })
              return !v
            })
          }
          onClose={() => setMenu(null)}

...
import { versionSupport, supportLabel } from '../../lib/k8sVersions'
  ...
  const rows: Array<[string, React.ReactNode]> = [
    ...
    [
      ...
      (() => {
        ...
        if (!v || !clusterInfo?.version) return clusterInfo?.version ?? '-'
        if (v.state === 'supported' || v.state === 'unknown') return clusterInfo.version
        return (
          <>
            {clusterInfo.version}{' '}
            <span className={`ver-badge is-${v.state}`} title={supportLabel(v)}>

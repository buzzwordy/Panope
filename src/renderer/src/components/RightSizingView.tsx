...
interface Props {
  ...
  /** open the pod behind a finding; tab preselects Logs for OOM kills */
  onOpen?: (pod: K8sObject, tab?: string) => void
}

export function RightSizingView({ namespace, contextVersion, onOpen }: Props): React.ReactElement {
            ...
            <tbody>
              {filtered.map((r) => (
                <tr
                  ...
                  className={onOpen ? 'is-clickable' : ''}
                  title={onOpen ? `Open pod ${r.namespace}/${r.pod}` : undefined}
                  onClick={() => {
                    if (!onOpen) return
                    const pod = pods.find(
                      (p) => p.metadata?.name === r.pod && p.metadata?.namespace === r.namespace
                    )
                    // OOM kills jump straight to the evidence; the rest to Status.
                    if (pod) onOpen(pod, r.issues.includes('oom-killed') ? 'logs' : undefined)
                  }}
                >

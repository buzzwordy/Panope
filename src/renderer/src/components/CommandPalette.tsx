...
export function CommandPalette({ items, onSelect, onClose }: Props): React.ReactElement {
  ...
  const filtered = useMemo(() => {
    ...
    // Rank: label prefix beats label substring beats group/key match, so
    // "pre" finds Preferences even though hundreds of objects also match.
    const scored: Array<{ i: PaletteItem; r: number }> = []
    for (const i of items) {
      const label = i.label.toLowerCase()
      let r = -1
      if (label.startsWith(s)) r = 0
      else if (label.includes(s)) r = 1
      else if (i.group.toLowerCase().includes(s) || i.key.toLowerCase().includes(s)) r = 2
      if (r >= 0) scored.push({ i, r })
    }
    scored.sort((a, b) => a.r - b.r)
    return scored.map((x) => x.i).slice(0, 50)
  }, [q, items])

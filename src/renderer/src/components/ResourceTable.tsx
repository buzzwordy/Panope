...
export function ResourceTable({
  ...
  function renderCell(col: ColumnDef, obj: K8sObject): React.ReactNode {
    ...
    const compute = COMPUTE[col.id]
    if (col.kind === 'ready') {
      const v = compute ? String(compute(obj)) : ''
      const m = /^(\d+)\/(\d+)$/.exec(v)
      if (!m) return <span>{v}</span>
      const total = Number(m[2])
      const state = total === 0 ? 'is-none' : Number(m[1]) >= total ? 'is-ok' : 'is-partial'
      return <span className={`ready-cell ${state}`}>{v}</span>
    }
    if (col.kind === 'status') {
      let sv: StatusValue

...
import { parseManifestRefs, scanApiVersions, type ApiFinding } from '../lib/apiCompat'
...
export function HelmDetailView({ def, obj, theme, readOnly = false, onBack, onChanged }: Props): React.ReactElement {
  ...
  const [apiFindings, setApiFindings] = useState<ApiFinding[] | null>(null)
  ...
  // Scanned on open rather than on the manifest tab: "will this release still
  // install after the next cluster upgrade" is worth knowing without hunting
  // for it. Discovery is the authority, so CRD versions count too.
  useEffect(() => {
    if (!isRelease) return
    let disposed = false
    Promise.all([api.helmGet(name, ns, 'manifest'), api.servedApiVersions()])
      .then(([manifest, served]) => {
        if (disposed || !served.length) return
        setApiFindings(scanApiVersions(parseManifestRefs(manifest), served))
      })
      .catch(() => undefined)
    return () => {
      disposed = true
  ...
  return (
    <>
      ...
      <div className="detail-body">
        ...
        {apiFindings && apiFindings.length > 0 && (
          <div className="api-warn">
            <div className="api-warn__head">
              <Icon name="event" size={14} />
              <b>
                {apiFindings.length} API version{apiFindings.length > 1 ? 's' : ''} this cluster no longer serves
              </b>
            </div>
            <div className="confirm-text">
              Rendered by this release but missing from cluster discovery. A re-install or upgrade will fail
              on these until the chart is updated.
            </div>
            <ul className="api-warn__list">
              {apiFindings.map((f) => (
                <li key={`${f.apiVersion}/${f.kind}`}>
                  <code>{f.apiVersion}</code> <span className="cell-muted">{f.kind}</span>
                  {f.alternatives.length ? (
                    <>
                      {' '}
                      <span className="cell-muted">-&gt; use</span> <code>{f.alternatives[0]}</code>
                    </>
                  ) : (
                    <span className="cell-muted"> - the whole group is gone from this cluster</span>
                  )}
                </li>
              ))}
            </ul>

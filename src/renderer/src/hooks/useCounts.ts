...
export interface CountEntry {
  ...
  namespaced: boolean
  /** the list was truncated, so byNs covers only the fetched page */
  partial?: boolean
}
    ...
    async function worker(): Promise<void> {
      while (!disposed && idx < defs.length) {
        ...
        try {
          ...
          setCounts((prev) => ({
            ...prev,
            [def.key]: { total: c.total, byNs: c.byNs, namespaced: def.namespaced, partial: c.partial }
          }))
        } catch {
...
export function displayCount(entry: CountEntry | undefined, namespace: string): number | undefined {
  ...
  // A truncated count only saw one page, so its per-namespace tally is a lower
  // bound. Showing no badge beats showing a number that is quietly wrong.

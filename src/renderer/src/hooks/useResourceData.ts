...
/**
 * `refreshSeconds` is a safety net, not the primary update path: lists ride a
 * live watch. A watch can stall quietly (a dropped connection that never
 * errors), and then the table silently shows the past. Re-listing on an
 * interval heals that. 0 disables it and leaves pure watch.
 */
export function useResourceData(
  ...
  // Set by the main effect; the interval effect calls it without re-subscribing.
  const relistRef = useRef<(() => void) | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    // Silent relist: refreshes the snapshot without touching loading state or
    // tearing down the watch, so the table does not blank out every tick.
    relistRef.current = () => {
      const p = def.custom ? api.listCustom(def.custom) : api.listResource(def.key)
      void p
        .then((res) => {
  ...
  // Own timer so changing the interval does not restart the watch. Keyed on
  // `key`, never the `def` object: def gets a new identity on re-render, which
  // would clear and restart the timer before it ever fired.
  useEffect(() => {
    if (!key || refreshSeconds <= 0) return
    const t = setInterval(() => relistRef.current?.(), refreshSeconds * 1000)
    return () => clearInterval(t)
  }, [key, refreshSeconds, contextVersion])

import { app, ipcMain, shell, webContents, type IpcMainInvokeEvent, type WebContents } from 'electron'
...
export function registerIpcHandlers(): void {
  ...
  // Watch every kubeconfig file; on change reload and tell the renderer so
  // rotated credentials / new contexts appear without a restart.
  const kubeconfigFiles = (process.env.KUBECONFIG ?? join(homedir(), '.kube', 'config'))
    .split(':')

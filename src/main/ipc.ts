...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC.aiConfirm, (_e, id: string, approve: boolean) => ai?.confirm(String(id), approve === true))
  ...
  ipcMain.handle(IPC.aiGetConfig, (): AiConfig | null => {
    ...
    return { provider: c.provider, baseUrl: c.baseUrl, model: c.model, hasKey: !!c.apiKey }
  })
  ipcMain.handle(IPC.aiSetConfig, (_e, cfg: { provider: string; baseUrl?: string; model: string; apiKey?: string }) => {
    const prev = aiSettings()
    const provider =
      cfg.provider === 'anthropic' ? 'anthropic' : cfg.provider === 'claude-code' ? 'claude-code' : 'openai'

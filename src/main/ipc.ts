...
import { checkForUpdate, latestK8sPatch } from './update'
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(IPC.openApiSchemas, (_e, apiVersion: string) => svc.openApiSchemas(String(apiVersion)))
  ipcMain.handle(IPC.servedApiVersions, () => svc.servedApiVersions())
  ...
  ipcMain.handle(IPC.aiGetConfig, (): AiConfig | null => {
    const c = aiSettings()
    if (!c) return null
    // claude-code is complete with every field empty (binary auto-discovered,
    // model defaulted by the CLI); the API providers need at least one field.
    if (c.provider !== 'claude-code' && !c.model && !c.apiKey && !c.baseUrl) return null
    return {
      provider: c.provider,
      baseUrl: c.baseUrl,
      model: c.model,
  ...
  ipcMain.handle(IPC.latestK8sPatch, (_e, minor: string) => latestK8sPatch(String(minor)))

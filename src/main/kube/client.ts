...
export class KubernetesService {
  ...
  private reloadConfig(): void {
    this.kc = new this.mod.KubeConfig()
  ...
  /** Count a resource (total + per-namespace) without shipping items to the renderer. */
  async countResource(key: string): Promise<{ total: number; byNs: Record<string, number> } | null> {
    ...
    return { total: res.items.length, byNs }
  }

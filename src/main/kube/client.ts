...
export class KubernetesService {
  ...
  private servedCache: string[] | null = null

  /**
   * Every group/version this cluster currently serves, e.g. ["v1", "apps/v1",
   * "gateway.networking.k8s.io/v1beta1"]. Discovery is the authority for
   * "does this apiVersion still exist here", which a static deprecation table
   * cannot answer for CRDs. Cached per context; reloadConfig clears it.
   */
  async servedApiVersions(): Promise<string[]> {
    if (this.servedCache) return this.servedCache
    const out = new Set<string>()
    try {
      const core = (await this.rawGet('/api')) as { versions?: string[] }
      for (const v of core?.versions ?? []) out.add(v)
    } catch {
      /* core discovery unavailable */
    }
    try {
      const apis = (await this.rawGet('/apis')) as {
        groups?: Array<{ name?: string; versions?: Array<{ groupVersion?: string }> }>
      }
      for (const g of apis?.groups ?? []) {
        for (const v of g.versions ?? []) if (v.groupVersion) out.add(v.groupVersion)
      }
    } catch {
      /* group discovery unavailable */
    }
    const list = [...out]
    // Only cache a real answer; an empty list means discovery failed and the
    // UI must not be told the cluster serves nothing.
    if (list.length) this.servedCache = list
    return list
  }
  ...
  async setContext(name: string): Promise<void> {
    ...
    // Another cluster serves another set of APIs and schemas.

...
/**
 * Latest patch release for a Kubernetes minor, from the project's own
 * per-minor pointer file (8 bytes, e.g. "v1.33.13"). Lives here next to the
 * Panope update check so every outbound request Panope makes is in one file.
 * Never throws; an unknown minor 404s and returns null.
 */
const k8sPatchCache = new Map<string, string | null>()

export async function latestK8sPatch(minor: string): Promise<string | null> {
  if (!/^\d+\.\d+$/.test(minor)) return null
  if (k8sPatchCache.has(minor)) return k8sPatchCache.get(minor) ?? null
  const value = await new Promise<string | null>((resolve) => {
    const req = get(
      `https://dl.k8s.io/release/stable-${minor}.txt`,
      { headers: { 'User-Agent': 'Panope' }, timeout: TIMEOUT_MS },
      (res) => {
        if (res.statusCode !== 200) {
          res.resume()
          resolve(null)
        ...
        res.on('end', () => resolve(/^v\d+\.\d+\.\d+/.test(body.trim()) ? body.trim() : null))
      }
    )
    req.on('timeout', () => req.destroy())
    req.on('error', () => resolve(null))
  })
  k8sPatchCache.set(minor, value)
  return value
}

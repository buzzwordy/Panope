...
export function runClaudeCode(opts: {
  ...
  cwd: string
  /**
   * Let the CLI use its own Bash/Read/Write/WebFetch tools. Those run inside
   * Claude Code, so they do NOT pass Panope's confirmation card or audit log.
   */
  ...
  const args = [
    ...
    mcpConfigPath,
    '--strict-mcp-config',
    // Headless mode denies anything not explicitly allowed, so unrestricted has
    // to name the built-ins rather than just drop the denylist. Those calls run
    // inside the CLI and Panope cannot see or gate them.
    '--allowedTools',
    opts.unrestricted ? `mcp__panope__*,${BUILTIN_TOOLS}` : 'mcp__panope__*',
    ...(opts.unrestricted ? [] : ['--disallowedTools', BUILTIN_TOOLS]),

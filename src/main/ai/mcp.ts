...
import type { AiToolCallReq, ChatTool } from './provider'
...
export interface McpBridge {
  ...
  /** Panope's tools plus any external ones, resolved per request. */
  tools(): ChatTool[]
}
    ...
    switch (method) {
      ...
      case 'tools/list':
        return {
          ...
          id,
          result: {
            tools: bridge.tools().map((t) => ({ name: t.name, description: t.description, inputSchema: t.parameters }))
          }
        }
      case 'tools/call': {

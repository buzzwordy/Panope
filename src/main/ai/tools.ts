...
/** Confirmation-card line for a tool Panope does not own. */
export function externalSummary(call: AiToolCallReq): string {
  const m = /^ext_([^_]+)_(.+)$/.exec(call.name)
  const where = m ? m[1] : 'external'
  const what = m ? m[2] : call.name
  const keys = Object.keys(call.args ?? {})
  return `${where}: ${what}${keys.length ? ` (${keys.slice(0, 4).join(', ')})` : ''}`
}

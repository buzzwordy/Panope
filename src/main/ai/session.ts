...
import type { AiEvent, AiUiContext, McpServerState } from '../../shared/types'
import { chat, type ProviderSettings, type Turn, type AiToolCallReq, type ChatTool } from './provider'
import { ALL_TOOLS, isMutation, runReadTool, runMutation, callSummary, externalSummary, type ToolDeps } from './tools'
...
- Tools named ext_<server>_<tool> come from an external MCP server the user added. They are not Panope and not the cluster. Use them when the task calls for it, and say which one you used.`

/** One line of situational context ahead of the user's text. */
function whereLine(ctx: AiUiContext): string {
  const obj = ctx.kind && ctx.name ? ` open object: ${ctx.kind} ${ctx.objNamespace ? `${ctx.objNamespace}/` : ''}${ctx.name}.` : ''
  return `[user is viewing: context=${ctx.context ?? '?'} namespace=${ctx.namespace ?? 'All'} view=${ctx.view ?? '?'}.${obj}]\n`
}
...
export class AiSession {
  ...
  // external MCP servers the user configured; their tools are always gated
  private ext = new McpClient()
  ...
  /** Panope's tools plus every connected external one. */
  allTools(): ChatTool[] {
    return [...ALL_TOOLS, ...this.ext.tools()]
  }

  externalStatus(): McpServerState[] {
    return this.ext.status()
  }

  /** Drop the connections so the next send picks up changed server config. */
  resetExternal(): void {
    this.ext.close()
    this.extStarted = false
  }

  private async ensureExternal(cfg: ProviderSettings): Promise<void> {
    if (this.extStarted) return
    this.extStarted = true
    const servers = cfg.mcpServers ?? []
    if (!servers.length) return
    await this.ext.start(servers)
    const bad = this.ext.status().filter((s) => s.error)
    for (const b of bad) this.emit({ type: 'error', error: `MCP server ${b.name}: ${b.error}` })
  }
  ...
  async send(text: string, ctx?: AiUiContext): Promise<void> {
    ...
    const where = ctx ? whereLine(ctx) : ''
    const gen = ++this.gen
    this.turns.push({ role: 'user', text: where + text })
    this.running = true
    this.abort = new AbortController()
    try {
      await this.ensureExternal(cfg)
      let round = 0
      for (; round < MAX_ROUNDS; round++) {
        const res = await chat(cfg, SYSTEM, this.turns, this.allTools(), (t) => this.emit({ type: 'text', text: t }), this.abort.signal)
  ...
  private async sendViaClaudeCode(text: string, cfg: ProviderSettings, ctx?: AiUiContext): Promise<void> {
    ...
    try {
      ...
      // loopback listener with a live token outlives the request. External
      // tools are re-exported through it rather than handed to the CLI
      // directly, so they still pass Panope's confirmation card.
      this.mcp = await startMcpServer(
        { runCall: (c) => this.runCall(c), tools: () => this.allTools() },
        this.appVersion
      )
      const where = ctx ? whereLine(ctx) : ''
      this.claudeRun = runClaudeCode({
        ...
        system:
          SYSTEM +
          (cfg.unrestricted
            ? '\nYou also have your own shell, file and web tools. Those run on the user\'s machine and are NOT gated by Panope, so prefer a Panope tool whenever one fits, and say what you are about to run before you run it.'
  ...
  private async runCall(call: AiToolCallReq): Promise<string> {
    const external = this.ext.isExternal(call.name)
    if (external) {
      // Panope cannot know what a third-party tool does, so it is gated
      // regardless of what the server claims about itself - unless the user
      // has explicitly chosen to stop being asked about this exact tool.
      const allowed = this.settings()?.allowedExternalTools ?? []
      if (!allowed.includes(call.name)) {
        const ok = await this.askApproval(call, true)
        if (!ok) return 'The user declined this action. Do not retry it unasked.'
      } else {
        this.emit({ type: 'tool', name: call.name, summary: externalSummary(call) })
      }
      return this.ext.call(call.name, call.args)
    }
    if (!isMutation(call.name)) {
      this.emit({ type: 'tool', name: call.name, summary: callSummary(call) })
      return runReadTool(call, this.deps)
    }
    const approved = await this.askApproval(call, false)
    if (!approved) return 'The user declined this action. Do not retry it unasked.'
    const res = await runMutation(call, this.deps)
    return JSON.stringify(res)
  }

  /** Show a confirmation card and wait for the click, a timeout, or a stop. */
  private askApproval(call: AiToolCallReq, external: boolean): Promise<boolean> {
    ...
    this.emit({
      ...
      id: key,
      name: call.name,
      summary: external ? externalSummary(call) : callSummary(call),
      args: call.args,

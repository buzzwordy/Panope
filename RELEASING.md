...
```sh
npm version 2.8.0 --no-git-tag-version   # then bump chart/Chart.yaml to match
git commit -am "v2.8.0" && git tag v2.8.0 && git push --follow-tags
```
...
3. publishes the GitHub release with every artifact attached. It is **not** a
   draft - a draft is invisible to anyone who is not a collaborator, so
   `/releases/latest` would 404 for the public and every download link would
   break. Pushing the tag is the point of no return; there is no second click.
...
`appVersion` agree and that the chart version is not already in GHCR (helm push
overwrites an OCI tag silently). `ci.yml` only covers pushes to `main`, so
without this a tag push had no gate at all.

...
```sh
npm version 3.3.0 --no-git-tag-version   # then bump chart/Chart.yaml to match
# and add a "## [3.3.0] - <date>" section to CHANGELOG.md
git commit -am "v3.3.0" && git tag v3.3.0 && git push --follow-tags
```
...
3. publishes the GitHub release with every artifact attached. The release text
   is that version's `CHANGELOG.md` section followed by the install notes. It
   is **not** a draft - a draft is invisible to anyone who is not a
   collaborator, so `/releases/latest` would 404 for the public and every
   download link would break. Pushing the tag is the point of no return; there
   is no second click.

Before any of that it runs a `verify` job: typecheck, unit tests, and an
assertion that the tag, `package.json` version and `chart/Chart.yaml`
`appVersion` agree, that `CHANGELOG.md` has a section for the tag, and that the
chart version is not already in GHCR (helm push overwrites an OCI tag
silently). `ci.yml` only covers pushes to `main`, so without this a tag push
had no gate at all.
...
disagree with the artifacts, and write the changelog entry:
...
| `CHANGELOG.md` | a `## [x.y.z] - YYYY-MM-DD` section; it becomes the release text |

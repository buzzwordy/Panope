...
The object's neighbourhood as a graph, and two objects side by side:

| Topology graph | Split view |
| --- | --- |
| ![Topology graph](docs/topology.png) | ![Split view](docs/split-view.png) |

The assistant investigating a failing pod, with every action gated:
...
```sh
sudo apt install ./panope_3.3.0_amd64.deb     # Debian / Ubuntu
sudo dnf install ./panope-3.3.0.x86_64.rpm    # Fedora / RHEL
chmod +x Panope-3.3.0.AppImage && ./Panope-3.3.0.AppImage
```

Windows: run `Panope-Setup-3.3.0.exe`, or unzip the portable build. macOS
...
- Tables update in real time and reconnect on their own when a watch drops, with
  an auto-refresh interval (default 10s, configurable) as a safety net in case a
  watch dies quietly.
...
- **View** - the object as editable YAML with field completion driven by the
  cluster's own OpenAPI schema, applied with server-side apply.
- **Describe** - the kubectl-describe essentials as one scannable pane.
- **Related** - the object's neighbourhood as a topology graph (or a tree, your
  choice): owners, managed ReplicaSets and pods, mounted ConfigMaps/Secrets/PVCs,
  and the Services and Ingresses exposing it. Click any node to open it.
- **Split** - pin the current object beside the list and keep browsing, so two
  objects sit side by side with their own tabs.
...
- **API removal scan** - opening a Helm release compares every rendered
  `apiVersion` against what your cluster's discovery actually serves, and flags
  the ones that are gone with the newest served replacement. Checked against the
  live cluster rather than a built-in table of Kubernetes deprecations, so it
  covers CRDs too: a chart pinned to an alpha CRD version breaks on an operator
  upgrade exactly like a removed built-in does.
- **Support window** - the Kubernetes version in the footer and About carries a
  badge once its minor stops getting patches, or is about to within 90 days. A
  minor newer than Panope's table reports nothing rather than guessing.
